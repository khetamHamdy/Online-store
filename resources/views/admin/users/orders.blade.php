                    ...
                    @forelse($items as $one)
                        <tr class="odd gradeX" id="tr-{{$one->id}}">
                            ...
                            <td class="v-align-middle wd-15p optionAddHours">
                                ...
                                <a data-order_id="{{$one->id}}" data-user_id="{{$item->id}}"
                                   class="btn btn-sm btn-bg-clean btn-icon download-pdf" title="{{__('cp.invoice')}}">
...
@section('js')
    ...
    <script>
        ...
        $(document).on('click', '.download-pdf', function () {
            ...
            console.log('start')
            // $(this).html('<div class="spinner-border text-success margin-50" role="status"> <span class="sr-only">Loading...</span> </div>');

            $.ajax({
                type: 'GET',
                url: "/admin/generate-invoice-pdf/" + order_id + "/" + user_id,
                data: {
                    'order_id': order_id,
                    'user_id': user_id,
                },
